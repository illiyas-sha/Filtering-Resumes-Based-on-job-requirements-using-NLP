...
from sklearn.feature_extraction.text import TfidfVectorizer #Convert a collection of text documents to a matrix of token counts
...
analyser = TfidfVectorizer().build_analyzer()

def stemmed_words(doc):

    #Lemmatize Sentence with the appropriate POS tag
    return (lemmatizer.lemmatize(word,tag.get_wordnet_pos(word))for word in analyser(doc) if word not in set(stopwords.words('english')))
